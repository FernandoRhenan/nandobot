  ...
  describe("Authorized user", () => {
    test("With a valid request", async () => {
      ...
      const response = await authenticatedFetch(
        `http://localhost:3000/api/v1/products/${productCreated.id}`,
      );
      ...
      expect(data).toStrictEqual<TCreatedProductAndUrl>({
        ...
        created_at: new Date(responseBody.data!.created_at).toISOString(),
        updated_at: new Date(responseBody.data!.updated_at).toISOString(),
        coupons: [
          {
            id: coupon.id,
            name: coupon.name,
